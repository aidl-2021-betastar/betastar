...
class BaseMovement1dEnv(SC2GameEnv):
    ...
    def _get_observation_space(self):
        ...
        space = spaces.Box(low=0, high=_PLAYER_RELATIVE_SCALE,
                           shape=screen_shape, dtype=np.int32)
        return space
    ...
    def _extract_observation(self, obs):
        ...
        obs = np.array(obs.reshape(self.observation_space.shape)) # type: ignore
        return obs

    def _translate_action(self, action):
        if action < 0 or action > self.action_space.n: # type: ignore
            return [_NO_OP]
        screen_shape = self.observation_spec[0]["feature_screen"][1:]
