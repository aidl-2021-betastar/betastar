...
from betastar.envs.env import ActionMask, Value
...
class Encoder(nn.Module):
    ...
    def forward(self, screens, minimaps, non_spatials):
        ...
        c = self.non_spatial((non_spatials + 1.0).log())
        return T.cat([a, b], dim=1), c


class FullNet(nn.Module):
    def __init__(self, env, hidden_size: int = 256, non_spatial_size: int = 32) -> None:
        super().__init__()
        ...
        self.backbone = nn.Sequential(
            nn.Linear(
                64 * env.spatial_dim * env.spatial_dim + non_spatial_size, hidden_size
            ),
        ...
        spatial_args_length = len(env.spatial_args_mask[env.spatial_args_mask == 1])
        self.spatial_policy = nn.Linear(hidden_size, spatial_args_length)
        self.non_spatial_policy = nn.Linear(
            hidden_size, self.action_space.sum() - spatial_args_length
        )
    ...
    def forward(
        ...
        latent_spatial, latent_non_spatial = self.encoder(
            screens, minimaps, non_spatials
        )
        latent = self.backbone(
            T.cat(
                [latent_spatial.view(latent_spatial.shape[0], -1), latent_non_spatial],
                dim=1,
            )
        )
